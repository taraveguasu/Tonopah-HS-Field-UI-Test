...
## 14. Do not restate what the specs and drawings already say — `high`

**This is the biggest single lesson from the first markup review (ITB-072, 08.11.26), and
it cut that exhibit from 46 scope items to 10.** His comments, verbatim:

> "These are deleted because it is duplicate information from the drawings and specifications."
> "Duplicate info. These are already stated in the Specifications"
> "Duplicate terms. These are all covered in the Terms & Conditions of the Subcontract Agreement."
> "Mobilizations already addressed in section below. Duplicate info"

Everything he struck was true, sourced and correctly cited. That was not the problem. The
problem is that an Attachment A which repeats the specification creates a second, weaker copy
of an obligation the Subcontract already imposes — and the two can drift.

Three categories to never write:

| Do not write | Because it lives in |
|---|---|
| Product construction: panel size, thickness, material, tactile character height, frame finish | the specification |
| Execution detail: mounting heights, ADA clearances, which side of the door, level and plumb | the specification and the drawings |
| Submittal procedure, testing, protection, cleanup, repair at completion, mobilizations | Division 01 and the Subcontract's own terms, and the PROJECT SPECIFIC PROVISIONS section below |

What survives is **the trade boundary** — what this subcontractor furnishes, what he
coordinates, and anything the documents *fail* to say. His one retained requirement of that
kind proves the rule: `Signage at exterior walls and all exterior locations shall be exterior
rated.` — kept, with the comment *"This remains because specifications only include interior
signage and no specs for exterior signs."* It is in the exhibit **because it is not in the
spec**.

Before writing any item, ask: is this already in a contract document the Subcontract
incorporates? If yes, it does not belong here. Rule 3's short median is a *symptom* of this
rule, not a separate instruction.

### The other half: what the AHJ will require and the documents omit

The same test run the other way is a reason to **add** an item, and it is the strongest reason
there is. Confirmed on ITB-072 (08.11.26): the PM rejected the narrative's "emergency
evacuation map" as unfounded, then put exit maps back on a different basis —

> "the fire marshal will require emergency exit maps posted in each room showing the exit
> route. So I decided to rewrite it."

Nothing in the drawings or the specification calls for them. The Fire Marshal will, at
inspection, and if the exhibit is silent it becomes a change order. Write these **naming the
authority** — *"as required by the Fire Marshal"*, *"as required by the AHJ"* — not
`as indicated`, because there is nothing indicated to point at.

So the two halves together: **an obligation belongs in the Attachment A when it is real and
not written down anywhere the Subcontract already reaches.** Duplicating the spec is the
common failure; missing the inspector's requirement is the expensive one.

### Keep the strike list — the RULE 14 REVIEW appendix

PM request 08.11.26, after seeing rule 14 applied to a package he had not marked up himself:

> "Rule 14 removes all duplicates from drawings and specs. Can you add a temporary section in
> the Att A that lists the duplicate items that you would have originally included? This lets me
> review the pertinent scope and I can choose if I want to add any of the duplicate items."

So rule 14 removes an item from the *exhibit*, not from the record. Draft the item, verify it,
then move it to `review_appendix` in the content record with the document that already carries
it. `build_attachment_a.py` renders those on a trailing page of the **draft**; `--final`
suppresses the page entirely, and Stage 7 exhibits must be built that way.

Group the struck items by where the obligation actually lives, because that is the question he
is answering:

| Group | What it means for him |
|---|---|
| **A. Already in the Project Manual or on the Drawings** | Safe to leave out. Restating creates a second, weaker copy |
| **B. Only in a Scope of Work narrative** | **The decision group.** If the narrative is not an exhibit to the executed Subcontract, striking these deletes the obligation rather than relocating it |
| **C. In no document at all** | Trade means and methods. Listed only so the list is complete |

Flag any item whose source documents conflict, and any the awarded sub has excluded in
writing — those are the ones most likely to come back as a change order.

The page is titled `DRAFT REVIEW ONLY - DELETE BEFORE ISSUE` in red, says on its own second
line that it is not Scope of Work, and each item carries a `☐` so it can be ticked in Bluebeam.
That labelling is not decoration: a list of obligations deliberately left out says the exact
opposite of what it means if it reaches a subcontractor unlabelled.

## 14a. Reference only what the subcontractor bid from — `high`

PM ruling 08.11.26:

> "The Exhibit B is a contract document between CORE and NCSD, not necessarily what the subs
> bid. I don't want it referenced in the Attachment A, but I do want the specific clause added
> as an inclusion/exclusion per the Exhibit. For example, the concrete scope should
> specifically exclude the concrete pad under existing visitors bleachers, but not reference
> the Basis of Proposal."

The DIRECTIVES list and any citation inside a scope item may name only documents that were
issued to bidders — addenda, clarifications, RFI responses, ASIs, PRs. Those are documents the
subcontractor priced against and can be held to.

**Owner-side agreements are different.** The Basis of GMP, the Owner's contract, internal
estimates and leveling sheets bind CORE, not the subcontractor. Naming one in an Attachment A
either incorporates a document the sub never saw, or invites him to read the rest of it.

So the clause goes in, and the source does not:

| Wrong | Right |
|---|---|
| "…in lieu of the cast-in-place headwall detail, **per Exhibit B - Basis of GMP (07.01.26) Owner Accepted Alternate**." | "…in lieu of the cast-in-place concrete headwall detail." |
| Directive line: "Exhibit B - Basis of Guaranteed Maximum Price (07.01.26)" | *(removed — the obligations it carries are written into the scope items and exclusions instead)* |
| "Excluded per the Basis of Proposal" | "Concrete pad under existing visitors bleachers" |

The obligation is identical either way; only the paper trail changes. **Read Exhibit B closely
when drafting** — it is where several real exclusions come from — then write what it decided,
never that it decided it.

## 15. Cite drawings by keynote — `medium`

> "Furnish and install 24 inch high exterior mounted, aluminum lettering per Keynote 6-04/A5-10."
> "Furnish and install exterior mounted Menu Display Case per Keynote 6-01/A5-10."

Format is `per Keynote <keynote>/<sheet>`. This is how he replaced a paragraph describing a
product: the keynote *is* the description. Two of ten items on ITB-072 carry one — a far higher
rate than the corpus-wide 1.2%, because two of the three products on that package are defined by
a keynote and nothing else.

## 16. Exclusions start empty — `high`

> "I generally don't list anything under exclusions unless the sub specifically asks for them to
> be listed. But I start with no listings."

He deleted 10 of 11 exclusions on ITB-072. The survivor was kept for a stated reason:

> "I kept this because this would typically be included by the signage sub"

So the test is **not** "is this outside the scope?" — almost everything is. It is: **would this
trade ordinarily assume it is theirs?** Mascot mural, yes: a signage subcontractor would expect
to price graphics. Concrete footings, flagpoles, scoreboards: no signage sub thinks those are
his, and listing them adds noise and invites the argument that anything unlisted is included.

This supersedes the default of writing a full exclusion list. Rule 12 still governs *how* an
exclusion is written when there is one — terse noun phrase, no routing.

## 17. The fixed sections have their own formats — `fixed`

Learned from the same markup; these are not voice, they are house format.

| Section | Format |
|---|---|
| Spec sections | **Number and description only** — `Section 10 14 23 - Panel Signage`. Not the template's "- Provide all labor, material, equipment, and services required to…" tail. Section numbers are spaced in pairs |
| Directives | `Addendum #01`, `Clarification #01`, `Clarification #02` — zero-padded, one per line, **no dates and no "including…" qualifiers** |
| ¶27 procurement log, ¶28 cost flow projections | **Always kept.** "These shall remain in all Attachment A's." Do not drop them as disproportionate on a small package |
| Group headers | Not bold, black text. The template's prototype is bold red because it is an instruction to the drafter |
| Address block | Left aligned, not justified |
| Mobilizations (¶94) | `multiple`, not a count |

## 18. Every product gets its own trade group — `high`

ITB-072 came back with **three** group headers — Building Signage, Exterior Lettering, Menu
Display Case — where the draft had collapsed everything into one plus a catch-all. Rule 13's
"small packages skip the structure" applies to packages with *one* thing in them, not to a small
dollar value. Three products means three groups even at $22,450.

Note what this did to the catch-all: with the duplicate-of-the-spec items gone (rule 14), there
was nothing left for a `General Scope Requirements` group to hold, and he deleted it. **The
closing group is for real residual obligations, not a place to park boilerplate.**
...
**D4. `Furnish and install` vs `Supply and install` — CLOSED 08.11.26. Use `Supply and
install`.** His markup used `Furnish and install` three times of three, which looked like it
contradicted rule 5. It does not: *"Go ahead and use Supply and install. I use that and furnish
and install interchangeably."* They are one construction with two spellings, so the corpus
majority decides — `Supply and install` at 10% against `Furnish and install` at 2%. Neither is
an error; `voice_check.py` flags only `Provide and install`, which is the blank template's verb
and the one he genuinely avoids.
