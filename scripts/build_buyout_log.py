...
OUT = ROOT / "04-output"
INDEX = ROOT / "01-index"
BUDGET = INDEX / "budget-phase-codes.json"
...
ATT_A_DRAFTED = {
    "RFP-008", "ITB-072", "RFP-031",
    # Batch 2, drafted 08.11.26 -- the next 8 by release priority.
    "RFP-033", "RFP-103", "RFP-002", "ITB-008",
    "ITB-089", "RFP-045", "RFP-030", "RFP-022",
}
...
def budget_codes():
    """package_id -> (phase codes, budget total) from the Procore budget."""
    if not BUDGET.exists():
        return {}
    return json.loads(BUDGET.read_text())["packages"]
...
def build():
    ...
    for pkg, drv in DRIVERS.items():
        ...
        rows.append(dict(
            ...
            phase_code=" / ".join(budget.get(pkg, {}).get("phase_codes", [])) or "—",
            budget_amount=budget.get(pkg, {}).get("budget_total"),
...
def write_workbook(rows):
    ...
    # ---- 1. Release priority -------------------------------------------
    hdr = ["Rank", "Tier", "Package", "Phase Code", "Title", "Carried Subcontractor",
           "Carried Value", "Binding Long-Lead Chain", "First Field Activity Needing This Sub",
           "Field Need Date", "Sub\nmit", "Rev\niew", "Proc\nure", "Exec", "Total",
           "Basis", "Attachment A\nDraft By", "Latest\nRelease Date", "Work Days\nof Slack", "Why This Rank"]
    w = [6, 22, 10, 17, 30, 24, 14, 34, 26, 12, 6, 6, 6, 6, 7, 10, 12, 13, 9, 62]
    data = []
    for r in rows:
        why = r["schedule_note"]
        if r["other_factors"]:
            why += "\n\n" + "\n".join("• " + f for f in r["other_factors"])
        data.append([r["priority_rank"], r["tier"], r["package_id"], r["phase_code"], r["title"],
    ...
    # ---- 2. Buy-out log -------------------------------------------------
    hdr = ["Package", "List", "Title", "Carried Subcontractor", "Phase Code", "Carried Value",
    ...
    w = [10, 20, 30, 24, 17, 14, 46, 34, 14, 20, 14, 14, 12, 12, 14, 13, 12, 6]
    data = []
    for r in rows:
        data.append([r["package_id"], r["source"], r["title"], r["carried_sub"],
                     r["phase_code"], r["carried_value"], r["gmp_lines"], r.get("group", r["combined"]), r.get("group_release_by", ""),
                     r["att_a_status"],
    ...
    tot = len(data) + 2
    ws.cell(tot, 5, "TOTAL CARRIED").font = BOLD
    ws.cell(tot, 6, sum(r["carried_value"] for r in rows)).font = BOLD
