...
# "model ODCC", "Model FB-2021", "no. PC64" -- a designation, not an abbreviation.
MODEL_DESIGNATION = re.compile(
    r"\b(?:model|series|type|no\.|number)\s+[A-Z][A-Z0-9-]{1,}\b", re.I)
...
def check_line(rep, where, text, is_exclusion=False):
    ...
    # A manufacturer's model designation is not an abbreviation and has nothing
    # to expand to -- "basis of design Aarco Products model ODCC". Rule 6 is
    # about trade terms of art, so allowlisting model numbers one at a time
    # would grow the list without bound and teach nothing.
    scan = MODEL_DESIGNATION.sub(" ", scan)
